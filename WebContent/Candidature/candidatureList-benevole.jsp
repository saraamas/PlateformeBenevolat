...
<h2 class="max-w-2xl mx-auto mt-9  text-2xl font-semibold mb-4">My applications</h2>
            ...
            <ul class="centers-list">
                <c:forEach var="candidature" items="${candidatureList}">
                     <div class="centers-container max-w-2xl mx-auto mt-5  bg-white rounded shadow-md" style="padding-top:10px; padding-bottom:10px; padding-left:30px;">
                
                    <li class="mb-4">
                        <strong class="block text-xl">${candidature.lettreMotivation}</strong>
                        <span class="text-gray-600">${candidature.statut}</span>
                        <a href="eventServlet?action=details&idEvent=${candidature.event.idEvent}" class="text-blue-500 hover:underline block mt-2">${candidature.event.titre}</a>
                    </li>
                    </div>
                </c:forEach>
            </ul>
